...
def inverse_zca_transformation_4d_torch(whitened_data, Vt, scale, mean):
    """
    Inverse ZCA transformation using memory-efficient components for a batch in PyTorch.
    
    Parameters
    ----------
    whitened_data : torch.Tensor
        ZCA-transformed data in shape (B, C, H, W).
    Vt : torch.Tensor
        The component matrix (eigenvectors transposed) with shape (n_components, features)
        where features == C*H*W.
    scale : torch.Tensor or float
        The scale factors (eigenvalues) for the transformation, shape (n_components,)
        or broadcastable.
    mean : torch.Tensor
        The mean that was subtracted before ZCA, shape (features,) or (1, features).
    
    Returns
    -------
    original_data : torch.Tensor
        Data reconstructed to the original space, with shape (B, C, H, W).
    """
    B, C, H, W = whitened_data.shape
    original_shape = whitened_data.shape

    # Flatten the data: shape (B, features)
    whitened_flat = whitened_data.view(B, -1)
    
    # Create a mask for non-NaN values
    mask = ~torch.isnan(whitened_flat)
    
    # Inverse transformation steps:
    # 1. Multiply by Vt.T to get to the "component space" (B, n_components)
    transformed = whitened_flat @ Vt.T
    
    # 2. Rescale (element-wise division by scale)
    rescaled = transformed / scale
    
    # 3. Multiply by Vt to go back to the original (centered) feature space
    original_centered = rescaled @ Vt
    
    # 4. Add the mean back to obtain the final reconstructed data
    original_data = original_centered + mean
    
    # Restore NaN positions if any
    original_data[~mask] = float('nan')
    
    # Reshape back to (B, C, H, W)
    original_data = original_data.view(original_shape)
